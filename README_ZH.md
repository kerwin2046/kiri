...
录屏完成后，打开 MP4 并选择「剪辑与导出」。在缩略图时间轴上点击定位、拖动片段两端裁剪，或在播放头处分割并删除中间片段；预览会跳过已删除部分。支持撤销、重做，以及按时段设置 1.5～4 倍局部放大和不透明隐私遮挡：在原始画面上拖动调整区域，再预览最终效果。保存会在素材库中创建新副本，保留原片。高画质保留原始尺寸；日常分享和小文件分别将最长边限制在 1080 和 720 像素，不放大小视频。导出保留声音，文件大小取决于素材和片段时长。Windows 的画面效果目前要求原视频不含旋转标记（Kiri 录屏符合此条件）。
