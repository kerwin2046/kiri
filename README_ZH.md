...
录屏完成后，打开 MP4，在视频上方常驻工具栏选择「剪辑与导出」。在缩略图时间轴上点击定位、拖动片段两端裁剪，或在播放头处分割并删除中间片段；预览会跳过已删除部分。裁剪、效果、标注及时间调整均支持撤销和重做。支持按时段设置 1.5～4 倍局部放大，可调整平滑进入和退出的时长；隐私遮挡提供模糊、像素和自定义颜色的纯色样式，可调强度。通过八个缩放手柄调整区域，并单独预览每段效果。截图中的画笔、矩形、直线、箭头、可编辑文字和像素／模糊马赛克笔刷也可用于视频，每个标注都有独立时间轨道，可拖动整段或两端调整出现时段。保存会在素材库中创建新副本，保留原片。导出画质菜单会说明各档用途并显示实际输出尺寸。高画质保留原始尺寸；日常分享和小文件分别将最长边限制在 1080 和 720 像素，不放大小视频。导出保留声音，文件大小取决于素材和片段时长。Windows 的画面效果目前要求原视频不含旋转标记（Kiri 录屏符合此条件）。播放控件位于画面外；倍速支持预设和 0.1～8 倍自定义输入，并记住播放偏好，不改变导出速度。
