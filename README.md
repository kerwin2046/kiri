...
- **Video editing**: open an MP4 and choose **Trim & Export**. Use the thumbnail timeline to seek, drag clip edges, split at the playhead, and delete unwanted sections. Preview skips removed sections; undo and redo cover cuts and effects. Add timed 1.5–4× zoom regions or opaque privacy masks, position them on the source frame, then preview the rendered framing. Save a new library copy without changing the original. High quality keeps source dimensions; Share and Small file cap the long edge at 1080 and 720 pixels without upscaling. Exports retain audio; file size depends on the source and selected duration. Windows effects currently require unrotated source video (including Kiri recordings).
