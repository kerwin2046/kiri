...
- **Video editing**: open an MP4 and choose **Trim & Export** in the persistent toolbar above the video. Use the thumbnail timeline to seek, drag clip edges, split at the playhead, and delete unwanted sections. Preview skips removed sections; undo and redo cover cuts, effects, annotation changes and timing. Add timed 1.5–4× zoom regions with adjustable smooth entry and exit, or blur, pixelated and custom-color solid privacy masks. Adjust intensity and eight resize handles on the source frame, then preview each effect. Screenshot tools—pen, rectangle, line, arrow, editable text, and pixel/blur mosaic brushes—are available as independent timed tracks. Drag each track or its edges to adjust when it appears. Save a new library copy without changing the original. The export menu explains each quality setting and displays its actual output dimensions. High quality keeps source dimensions; Everyday sharing and Compact file cap the long edge at 1080 and 720 pixels without upscaling. Exports retain audio; file size depends on the source and selected duration. Windows effects currently require unrotated source video (including Kiri recordings). Playback controls stay outside the picture; choose a speed preset or enter 0.1–8×, remembered for playback only.
