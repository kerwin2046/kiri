...
import {validVideoEffect,type VideoEffect} from "./video-effects";
...
export function VideoOutputEffectTracks(props:Props){
  ...
  const label=(effect:VideoEffect)=>props.labels[effect.id]??t(effect.kind==="zoom"?"Zoom":"Privacy mask");
  function begin(event:PointerEvent<HTMLElement>,effect:VideoEffect,range:ProjectedRange,mode:Mode){
    ...
    let latest=original;gesture.current=true;target.focus();target.setPointerCapture(event.pointerId);props.onSelect(effect.id);props.onSeek(range.sourceStart);
    const move=(e:globalThis.PointerEvent)=>{
      ...
      latest=original.map(item=>item.id===effect.id?next:item);props.onChange(latest,true);
    };
  ...
  return <><div className="kiri-video-effect-tracks" aria-label={t("Effect tracks")}>
    {rows.filter(row=>row.ranges.length).map(({effect,ranges})=><div key={effect.id} className="kiri-video-effect-track">
      <span className="kiri-video-lane-label">{props.labels[effect.id]?<Pencil size={12}/>:effect.kind==="zoom"?<Focus size={12}/>:<Shield size={12}/>}<span>{label(effect)}</span></span>
      {ranges.map(range=><div key={range.index} className="kiri-video-effect-bar" data-selected={effect.id===props.selectedId} style={{left:`${range.start/total*100}%`,width:`${(range.end-range.start)/total*100}%`}}>
        <button type="button" className="kiri-video-effect-bar-body" disabled={props.disabled} aria-pressed={effect.id===props.selectedId} aria-label={`${label(effect)} · ${range.start.toFixed(1)}–${range.end.toFixed(1)} s`}
          onPointerDown={event=>begin(event,effect,range,"move")} onClick={event=>{if(event.detail===0){props.onSelect(effect.id);props.onSeek(range.sourceStart);}}}
